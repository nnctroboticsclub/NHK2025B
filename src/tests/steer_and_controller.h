...
std::array<PidParameter, NUM_OF_ROBOMAS> robomas_pid_param{
    []{ PidParameter p; p.kp = 0.6,p.kd = 0.101, p.out_max = 2.0; return p;}(),
    []{ PidParameter p; p.kp = 0.6,p.kd = 0.101, p.out_max = 2.0; return p;}(),
    []{ PidParameter p; p.kp = 0.6, p.kd = 0.101; return p;}(),
    []{ PidParameter p; p.kp = 0.6, p.kd = 0.101; return p;}(),
};
...
int main()
{
    ...
    float back_currentR = 0, back_currentL = 0;
    ...
    while (true)
    {
        ...
        // ES = puropo.getCommunicatable(0) & (puropo.getAxis(0, 10) == 1);
        ...
        front_omegaR = -puropo.getRightY(0) * 5 * ((puropo.getAxis(0,8) == 1)?HIGH_SPEED:(puropo.getAxis(0,8) == -1)?LOW_SPEED:NOMAL_SPEED);
        front_omegaL = puropo.getLeftY(0) * 5 * ((puropo.getAxis(0,8) == 1)?HIGH_SPEED:(puropo.getAxis(0,8) == -1)?LOW_SPEED:NOMAL_SPEED);
        back_omegaR = -puropo.getRightY(0) * 5 * ((puropo.getAxis(0,8) == 1)?HIGH_SPEED:(puropo.getAxis(0,8) == -1)?LOW_SPEED:NOMAL_SPEED);
        back_omegaL = puropo.getLeftY(0) * 5 * ((puropo.getAxis(0,8) == 1)?HIGH_SPEED:(puropo.getAxis(0,8) == -1)?LOW_SPEED:NOMAL_SPEED);

        robomas_pid.setGoalValue(0,front_omegaL);
        robomas_pid.setGoalValue(1,front_omegaR);
        ...
        robomas.setCurrent(2, back_currentL = robomas_pid.getOutput(2));
        robomas.setCurrent(3, back_currentR = robomas_pid.getOutput(3));
        update();
