...
{
    time_r[1] = timer.read_ms();
    if (time_r[1] - time_r[0] > 1000)
        can_flag_read = 0;
...
int ikarashiCAN_mk2::get_send_flag()
{
    int ret_val = 1;
    if (can_flag_send)
        time_s[0] = timer.read_ms();
    time_s[1] = timer.read_ms();
    if (time_s[1] - time_s[0] > 1000)
        ret_val = 0;
    return ret_val;
}
...
void ikarashiCAN_mk2::read_start()
{
    ...
    can.attach(Callback<void()>(this, &ikarashiCAN_mk2::onErrorPassive), CAN::EpIrq);
    can.attach(Callback<void()>(this, &ikarashiCAN_mk2::onBusError), CAN::BeIrq);
}

void ikarashiCAN_mk2::read_for_attach()
{
    time_r[0] = timer.read_ms();
...
void ikarashiCAN_mk2::onErrorPassive()
{
    this->reset();
    ThisThread::sleep_for(1ms);
}

void ikarashiCAN_mk2::onBusError()
{
    this->reset();
    ThisThread::sleep_for(1ms);
}

int ikarashiCAN_mk2::get_tderror()
{
    return can.tderror();
}

int ikarashiCAN_mk2::get_rderror()
{
    return can.rderror();
}

void ikarashiCAN_mk2::update(int error_handle)
{
    if (this->get_tderror() >= error_handle)
    {
        this->reset();
        ThisThread::sleep_for(1ms);
    }
